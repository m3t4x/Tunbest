...
mongoose.connect('mongodb+srv://Cluster35362:blVKRmJFc0th@cluster35362.5ckei5v.mongodb.net/?retryWrites=true&w=majority', {
    useNewUrlParser: true,
    useUnifiedTopology: true
}, err => err ? console.log(err) :
    console.log('Connected to yourDB-name database'));
var fs = require('fs');
var movie = require("./model/product.js");
...
/* Api to add Product */
app.post("/add-movie", upload.any(), (req, res) => {
  try {
    if (req.files && req.body && req.body.name && req.body.desc) {

      let new_product = new movie();
        ...
        product.find(query).count()
          .then((count) => {
            ...
            if (data && data.length > 0) {
              ...
              res.status(400).json({
                errorMessage: 'There are no movies!',
                status: false
              });
            }
